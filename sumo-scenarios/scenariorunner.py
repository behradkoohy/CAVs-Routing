...
DEPLOY_SANDBOX = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "DEPLOY_SANDBOX"
)
...
def get_options():
    ...
    optParser.add_option(
        "-a", "--additional", help="additional argument to pass to the scenario"
    )
...
if stddefs.options.deploy:
    ...
    deploy(
        stddefs.options.dest,
        stddefs.options.verbose,
        scenariodefs.toRemove,
        scenariodefs.toDeploy,
    )
else:
    sys.path.append(".")
    import scenariodefs
