...
def get_options(args=None):
    ...
    argParser.add_argument(
        "--demand",
        help="the demand file(s)",
        default="../demand/osm_activitygen_present_no_prt.rou.xml.gz,../demand/osm_activitygen_future_no_prt.rou.xml.gz,../demand/osm_activitygen_future_with_prt.rou.xml.gz",
    )
    argParser.add_argument(
        "--tripinfo",
        help="the tripinfo file(s)",
        default="../osm/outputs/present_tripinfo.out.xml,../osm/outputs/future_tripinfo.out.xml,../osm/outputs/prt_tripinfo.out.xml",
    )
    argParser.add_argument(
        "--net", help="the net file", default="../prt_infra/joined.net.xml"
    )
    argParser.add_argument(
        "--roi",
        help="the region of interest label:lon,lat:radius",
        default="Klinikum:9.71452481,50.87592257:300",
    )
...
def find_roi_persons(region, demand):
    ...
    for person in parse_fast_structured(
        demand, "person", ["id"], {"ride": ["lines"], "stop": ["lane"]}
    ):
...
def parse_persons(tripinfo, pids):
    travelSpeed = Statistics("travel speeds", histogram=True, scale=5.0)
    invalid = 0
    attrs = {
        "ride": ["duration", "routeLength"],
        "walk": ["duration", "routeLength"],
        "stop": ["duration"],
    }
    for person in parse_fast_structured(
        tripinfo, "personinfo", ["id", "depart"], attrs
    ):
...
def parse_vehicles(tripinfo, vids):
    ...
    sumCO2 = 0
    for v in parse_fast_structured(
        tripinfo, "tripinfo", ["id", "routeLength"], {"emissions": "CO2_abs"}
    ):
        if vids is not None and v.id not in vids and "prt." not in v.id:
            continue
        if "prt." in v.id:
            travelDist["prt"] += float(v.routeLength) / 1000.0
        elif "." in v.id:
            travelDist["bus"] += float(v.routeLength) / 1000.0
        else:
            travelDist["car"] += float(v.routeLength) / 1000.0
        if v.emissions:
...
if __name__ == "__main__":
    ...
    if options.roi:
        ...
        region = set(
            [
                lane.getID()
                for lane, dist in net.getNeighboringLanes(x, y, float(radius))
            ]
        )
        poi = []
        voi = []
