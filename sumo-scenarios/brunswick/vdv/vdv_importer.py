...
def get_options(args=None):
    ...
    optParser.add_option(
        ...
        default=1,
        type=int,
        help="The type of day you want to extract schedule info for.",
    )
    optParser.add_option(
        "--use-lines",
        dest="uselines",
        default="all",
        help='Apply a filter on which lines to convert from the VDV data (comma separated). Default is "all".',
    )
    optParser.add_option(
        "--ignore-lines",
        dest="ignorelines",
        default="",
        help='Apply a filter on which lines not to convert from the VDV data (comma separated). Default is "".',
    )
    optParser.add_option(
        "--departure-offset",
        dest="departureOffset",
        default=0,
        type=int,
        help="An offset (in seconds) to the departures read from the VDV data",
    )
    optParser.add_option(
        "--generate-umlauf",
        dest="generateUmlauf",
        action="store_true",
        help='If "true", no single vehicle trip for each VDV trip is generated, but a complete umlauf (succession of trips) over the whole day.',
    )
    optParser.add_option(
        "--stop-patch-file",
        dest="stopPatchFile",
        help="A file that contains patches for localization of stops on specific lanes.",
    )
...
def dms2dd(string):
    ...
    if len(minutes) > 0:
        result += float(minutes) / 60.0
    if len(seconds) > 1:
        result += float(seconds) / 3600.0
    return result
...
def read_functional_place_types():
    ...
    print(
        " INFO: Read %s functional place types from %s" % (len(result), FILE_ONR_TYPE)
    )
    return result
...
def read_sections(places):
    ...
    for c in content:
        ...
        if fromId in [k[0] for k in places.keys()] and toId in [
            k[0] for k in places.keys()
        ]:
...
def read_routes(use_lines, ignore_lines):
    ...
    for c in content:
        line_nr = c[1].strip()
        if (
            use_lines == "all" or line_nr in use_lines.split(",")
        ) and not line_nr in ignore_lines.split(","):
...
def read_trips(vtypes, daytype, use_lines, ignore_lines):
    ...
    for c in content:
        if int(c[3]) == daytype:
            ...
            line_id = c[2].strip()
            if (
                use_lines == "all" or line_id in use_lines.split(",")
            ) and not line_id in ignore_lines.split(","):
...
def read_umlaeufe(daytype):
    ...
    for c in content:
        if int(c[0]) == daytype:
            ...
            uniqueIDs.add(uid)
    print(
        " INFO: Read %s umlaeufe from %s (%s unique items)"
        % (len(umlaeufe), FILE_UMLAUF, len(uniqueIDs))
    )
    return umlaeufe
...
def create_trips(routes, trips, umlaeufe, net, generateUmlauf):
    ...
    for (
        trip_id,
        (depart, line_id, line_var_id, fgr_nr, mode, umlauf_id),
    ) in trips.items():
        ...
        if len(stops) < 2:
            ...
            if warnCounter < 10:
                print(
                    "Too few stops for trip %s on line %s. Ignoring."
                    % (trip_id, line_id)
                )
            continue
        ...
        if generateUmlauf:
            if not umlauf_id in veh_trips:
                veh_trips[umlauf_id] = []
            veh_trips[umlauf_id].append(
                (depart, veh_type, edges, routes[(line_id, line_var_id)], fgr_nr)
            )
        else:
            veh_trips[identifier] = (
                depart,
                veh_type,
                edges,
                routes[(line_id, line_var_id)],
                fgr_nr,
            )
...
def write_route_file(
    vTypes, vehicles, routes, trips, stop_times, stop2edge, generateUmlauf
):
    ...
    with open("transit.rou.xml", "w") as f:
        ...
        if generateUmlauf:
            ...
            for trip_id, (depart, veh_type, edges, stops, fgr_nr) in sorted_trips:
                ...
                f.write(
                    '    <trip id="%s" type="%s" depart="%s" from="%s" to="%s" >\n'
                    % (trip_id, veh_type, depart + OFFSET, fromEdge, toEdge)
                )
                for stop in stops:
                    stop_time = stop_times.get(
                        (stop, fgr_nr.strip()), DEFAULT_STOP_TIME
                    )
                    f.write(
...
def generateAndWriteUmlaeufe(outf, vehicles, routes, trips, stop_times, stop2edge):
    ...
    for umlauf_id in trips:
        ...
        if vehicle is not None:
            ...
            print(
                " Warning: Could not find a vehicle for umlauf %s (needed type: %s)! Creating virtual vehicle."
            ) % (umlauf_id, veh_type)
    ...
    for umlauf_id, umlaufList in trips.items():
        ...
        tripStart = False
        firstDep = 0
        for depart, veh_type, edges, stops, fgr_nr in sorted(
            umlaufList, key=operator.itemgetter(0)
        ):
            stops = [stop_id for stop_id in stops if stop_id in stop2edge]
            if len(stops) < 2:
                continue
            ...
            if not tripStart:
                ...
                umlaeufe[(firstDep, vehicle)] += (
                    '    <trip id="%s" type="%s" depart="%s">\n'
                    % (vehicle, veh_type, depart + OFFSET)
                )
            else:
                umlaeufe[(firstDep, vehicle)] += (
                    '        <stop lane="%s" endPos="0.2" until="%s" parking="true" />\n'
    ...
    for (depart, vehicleId), string in sorted(
        umlaeufe.items(), key=operator.itemgetter(0)
    ):
        outf.write(string)
...
def map_stops_to_lanes(places, net, stop2Modes):
    ...
    for (pid, ptype), (name, lon, lat) in places.items():
        ...
        # print(ptype, x,y)
        # print(bbox)
        if ((int(ptype) < 3 and pid in stop2Modes) or int(ptype) > 6) and (
            x >= bbox[0][0] and x <= bbox[1][0] and y >= bbox[0][1] and y <= bbox[1][1]
        ):
            # print('x=%s y=%s' % (x,y))
            ...
            if len(lanes) < 1:
                ...
                # if warnCounter < 10:
                print(
                    "Could not find a lane near stop %s (%s, type=%s)!"
                    % (pid, name, ptype)
                )
                continue
                ...
                for lane in lanes:
                    ...
                    if sum(allowed) == len(modes):
                        ...
                        if linkIndex > 0:
                            ...
                            if sum([testLane.allows(mode) for mode in modes]) == len(
                                modes
                            ):
                                l = testLane
...
def create_charging_station_string(pid, name, lane, pos, max_cap=200):
    ...
    if minPos < STOP_LENGTH:
        minPos = STOP_LENGTH
    return (
        '    <chargingStation id="%s" chargeDelay="2" chargeInTransit="1" power="%s" efficiency="0.95" endPos="%s" lane="%s" startPos="%s" friendlyPos="true" />\n'
...
def create_route_fcd_file(places, routes):
    with open("bus_routes.xml", "w") as outf:
        ...
        outf.write(
            '    <timestep time="%s"><vehicle id="%s_%s" x="%s" y="%s" until="%s" name="%s" fareZone="%s" fareToken="%s" startToken="%s" speed="20"/></timestep>\n'
        )
        outf.write("</fcd-export>")
        ...
        for (line_nr, line_var), stops in routes.items():
            ...
            for stop in stops:
                ...
                if (id, type) in places.keys():
                    ...
                    outf.write(
                        '        <trkpt lon="%s" lat="%s"><time>%s</time></trkpt>\n'
                        % (lat, lon, 0)
                    )
...
if __name__ == "__main__":
    ...
    veh_trips, stop2Modes = create_trips(
        routes, trips, umlaeufe, network, options.generateUmlauf
    )
