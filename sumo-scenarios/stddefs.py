...
def getDefaults(netfile, tlsfile):
    if tlsfile != "":
        build = buildProcess + [
            get_python_tool("tls/tls_csv2SUMO.py")
            + " %s %s > %s"
            % (",".join(sorted(glob.glob("data/*.csv"))), netfile, tlsfile)
        ]
...
def get_python_tool(rel_path):
    return (
        ...
        + '" "'
        + os.path.join(os.environ["SUMO_HOME"], "tools", rel_path)
        + '"'
    )
...
def filterLog(
    log="data/sumo_log.txt",
    statsOut="data/stats.txt",
    statsIn="stats.scenario",
    tripinfos="data/tripinfos.xml",
):
    collisions = 0
    timeout = 0
    simEnd = -1
    ...
    if os.path.exists(tripinfos):
        ...
        durationStats = ""
    statLine = "Collisions: %s Timeouts: %s End: %s%s" % (
        collisions,
        timeout,
        simEnd,
        durationStats,
    ...
    with open(statsOut, "w") as o:
        ...
        oldStats = os.path.join(
            os.environ["TEXTTEST_ROOT"],
            os.environ["TEXTTEST_SANDBOX"][testNameStart:],
            statsIn,
        )
        if os.path.exists(oldStats):
            ...
            if line.strip() == statLine.strip():
                o.close()
                return
        sumoVersion = subprocess.check_output(
            get_app("sumo", "SUMO_BINARY") + " -V", shell=True
        ).splitlines()[0]
        print("%s %s\n%s" % (datetime.now(), sumoVersion, statLine), file=o)


buildProcess = [
    "%s -c data/build.netc.cfg" % get_app("netconvert", "NETCONVERT_BINARY")
]
runProcess = [
    "%s -c data/run.sumo.cfg --no-step-log --no-duration-log"
    % get_app("sumo", "SUMO_BINARY"),
    filterLog,
]
